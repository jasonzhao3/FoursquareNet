...
//Here I made some modification to adapt it for the weighted graph
...
namespace TSnap {
    ...
    namespace TSnapDetail {
        ...
        private:
            struct TCmtyDat {
                ...
                int MxQId;
                TCmtyDat() : MxQId(-1) { }
                TCmtyDat(const double& NodeWeightFrac, const int& Weight) :
                WeightFrac(NodeWeightFrac), NIdQH(Weight), MxQId(-1) { }
        ...
        public:
            TCNMQMatrix(const PUNGraph& Graph, const THash<TInt, TInt> & NodeWeight) : CmtyQH(Graph->GetNodes()),
            MxQHeap(Graph->GetNodes()), CmtyIdUF(Graph->GetNodes()) { Init(Graph, NodeWeight); }
            
            void Init(const PUNGraph& Graph, const THash<TInt, TInt> & NodeWeight) {
                //Calculate Total Weight
                globalCounter = 0;
                double m = 0;
                for (THash<TInt, TInt>::TIter HI = NodeWeight.BegI(); HI != NodeWeight.EndI(); HI++) {
                    m += NodeWeight.GetDat(HI->Key);
                }
                
                const double M = 0.5/m; // 1/2m
                Q = 0.0;
                for (TUNGraph::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
                    CmtyIdUF.Add(NI.GetId());  //disjoint-set
                    const double CurrWeight = NodeWeight.GetDat(NI.GetId());
                    if (CurrWeight == 0) { continue; } //impossible
                    TCmtyDat& Dat = CmtyQH.AddDat(NI.GetId(), TCmtyDat(M * CurrWeight, CurrWeight));
                    for (int e = 0; e < NI.GetOutDeg(); e++) {
                        const int DstNId = NI.GetOutNId(e);
                        //change degree k to node weight
                        const double DstMod = 2 * M * (1.0 - CurrWeight * NodeWeight.GetDat(Graph->GetNI(DstNId).GetId()) * M);
                        Dat.AddQ(DstNId, DstMod);
                    }
                    Q += -1.0*TMath::Sqr(CurrWeight*M);
            ...
            bool MergeBestQ() {
                ...
                { TCmtyDat& DatI = CmtyQH.GetDat(I);
                    ...
                    for (int i = -1; DatJ.NIdQH.FNextKeyId(i); ) {
                        ...
                        else { NewQ = NewQ-2*DatI.WeightFrac*DatK.WeightFrac; }  // K connected to J not I
                        DatJ.AddQ(K, NewQ);
                        DatK.AddQ(J, NewQ);
                    ...
                    for (int i = -1; DatI.NIdQH.FNextKeyId(i); ) {
                        ...
                        if (! DatJ.NIdQH.IsKey(K)) { // K connected to I not J
                            TCmtyDat& DatK = CmtyQH.GetDat(K);
                            const double NewQ = DatI.NIdQH[i]-2*DatJ.WeightFrac*DatK.WeightFrac; 
                            DatJ.AddQ(K, NewQ);
                            DatK.DelLink(I);
                            DatK.AddQ(J, NewQ);
                            MxQHeap.PushHeap(TFltIntIntTr(NewQ, TMath::Mn(J,K), TMath::Mx(J,K)));
                    ...
                    DatJ.WeightFrac += DatI.WeightFrac; }
                if (DatJ.NIdQH.Empty()) { CmtyQH.DelKey(J); } // isolated community (done)
                CmtyQH.DelKey(I);
                globalCounter++;
                printf("finish one more merge %d, hoho!\n", globalCounter);
                return true;
            }
            static double CmtyCMN(const PUNGraph& Graph, TCnComV& CmtyV, const THash<TInt, TInt> & NodeWeight) {
                TCNMQMatrix QMatrix(Graph, NodeWeight);
                // maximize modularity
    ...
    double CommunityCNM(const PUNGraph& Graph, TCnComV& CmtyV, const THash<TInt, TInt> & NodeWeight) {
        printf("haha\n");
        return TSnapDetail::TCNMQMatrix::CmtyCMN(Graph, CmtyV, NodeWeight);
    }
